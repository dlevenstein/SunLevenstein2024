function [simresults,parms] = Run_SynHomeo2_CdV(manip,parms,timeparms,varargin)
...
DefaultParms.kvf = 2e-2;        %Association rate of CamKII (s^-1)
DefaultParms.kvd_bar = 3e-1;    %Maximal dissocation rate of CamKII (s^-1)
DefaultParms.kvd_l = 10;        %Rate-decay constant for CamKII dissociation (Hz)
...
for tt = 2:length(timesteps)
    ...
    kvd = kvd_bar.*exp(-R(tt-1)./kvd_l);
	dvdt = kvf.*(1-v(tt-1)) - kvd.*v(tt-1);
    ...
    kf(tt) = (kf_0.*blockM_base(tt) + k_CamK.*m(tt).*v(tt).*blockM(tt).*Autophos);
